defmodule IntcodeCpu do
  defmodule Decoder do
    def instruction(program, pc) do
      ...
      case base_instruction(word) do
        ...
        {:two_arg, opcode} -> {opcode, 3, operand(program, pc, :first), operand(program, pc, :second)}
  ...
  defmodule Executor do
    ...
    def execute(program, pc, {:jt, pc_offset, op1, op2}) do
      v1 = fetch(program, op1)
      v2 = fetch(program, op2)
      if v1 != 0 do
        {:continue, v2, program}
    ...
    def execute(program, pc, {:jf, pc_offset, op1, op2}) do
      v1 = fetch(program, op1)
      v2 = fetch(program, op2)
      if v1 == 0 do
        {:continue, v2, program}
    ...
    def execute(program, pc, {:lt, pc_offset, op1, op2, target}) do
      v1 = fetch(program, op1)
      v2 = fetch(program, op2)
      result = if v1 < v2, do: 1, else: 0
      new_program = store(program, target, result)
      {:continue, pc + pc_offset, new_program}
    end

    def execute(program, pc, {:eq, pc_offset, op1, op2, target}) do
      v1 = fetch(program, op1)
      v2 = fetch(program, op2)
      result = if v1 == v2, do: 1, else: 0
      new_program = store(program, target, result)
